...
The TUI is a client only — it needs a LangGraph Deep Agent server reachable at `LANGGRAPH_URL` (default `http://localhost:2024`). Start one in another terminal with `uv run langgraph dev --no-browser` from the agent project. See `docs/configuration.md` for env vars (`LANGGRAPH_URL`, `GRAPH_ID`, `THREAD_ID`, `LANGSMITH_API_KEY`). Per-directory `.env` is loaded automatically; parent dirs are not searched.

State written outside the repo:
- `~/.deepagent-tui/threads.db` — SQLite thread index (`/resume`); the picker is scoped to the current `graph_id` + `workspace`
- `~/.deepagent-tui/config.toml` — persisted settings (theme, HITL, tool-widget mode, …). A top-level default layer plus per-agent `[graph."<graph_id>"]` overrides; `/settings` writes to the connected agent's section, so preferences don't leak across agents
