...
`tests/test_tui_smoke.py` boots the app with stubbed bootstrap (no server needed) and asserts that the layout mounts and basic interactions don't blow up. It has grown beyond the original layout smoke checks to also cover the settings modal, the `@`/`!` input prefixes, file-reference rewriting, input-history recall, and paste handling — a couple dozen tests, all sub-second.

A representative slice of the core layout/wiring tests:
