...
Built-in commands take precedence over dynamic skill commands. If a skill is named `help`, `new`, `clear`, `exit`, `resume`, `rewind`, `compact`, `copy`, `export`, `settings`, or `skills`, the built-in wins and the skill is unreachable as a slash command (but still loaded server-side).
